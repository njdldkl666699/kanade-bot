...
async def _(event: OneBotMessageEvent, arg_msg: Message = CommandArg()):
    ...
    try:
        prompt, size, ratio, prompt_extend = parse_image_args(arg_msg.extract_plain_text())
        images = await create_image(prompt, size, ratio, prompt_extend)
    except CreationError as exc:
        await image_generation.finish(str(exc))

    succeed_consume(*kpu)
    await image_generation.finish(compose_images(event.message_id, images))
...
async def _(
    ...
    try:
        image_urls = await image_data_urls(bot, event.reply.message if event.reply else None)
        image_urls.extend(await image_data_urls(bot, event.message))
    except CreationError as exc:
        await image_edit.finish(str(exc))
    if image_urls:
        try:
            images = await edit_image(image_urls, prompt, size, ratio, prompt_extend)
        except CreationError as exc:
            await image_edit.finish(str(exc))
        succeed_consume(*kpu)
        await image_edit.finish(compose_images(event.message_id, images))
...
async def _(
    ...
    try:
        prompt, seconds, ratio, mode = parse_video_args(arg_msg.extract_plain_text())
    except CreationError as exc:
